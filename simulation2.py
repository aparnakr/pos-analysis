...
inflation = [1, 7, 14]
with open('monte_carlo.csv', 'w') as csvfile:
...
for i in range(3):
	...
	while(t <1000000000):
	    #per block inflation for 5% per year
	    #one second block t
	    ...
	    #print("fees: " + str(fees) + "reward" + str(reward))
	    ...
	    if(t%1000000 == 0):
	        ...
	        dates.append(t)
	        #['block_number', 'adv_stake_percent', 'inflation_rate', 'start_staked_percent', 'bonded_percent']
	        writer.writerow({'block_number': t, 'adv_stake_percent': 100*prop, 'inflation_rate': inflation[i], 'start_staked_percent': 25, 'bonded_percent': 10 })
	    t+=1
